...
import StudentSidebar, {
  studentMenuLabels,
  type StudentMenuKey,
} from "./_components/student-sidebar";
...
const StudentDashboardContent = () => {
  ...
  const handleMenuChange = (key: StudentMenuKey | "logout") => {
    ...
    params.set("menu", key);
    router.push(`/student/student_dashboard?${params.toString()}`);
  };
  ...
  return (
    <div className="flex min-h-screen gap-4 bg-[#f3f5f9] p-4">
      <StudentSidebar activeKey={activeMenu} onNavigate={handleMenuChange} />
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200/70 bg-white/90 px-6 py-4 backdrop-blur-xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-violet-400 to-violet-600 text-xs font-bold text-white shadow-sm">
                HV
              </div>
            </div>
          </div>
              ...
              <div className="relative mx-auto max-w-5xl space-y-10">
                ...
                <section className="flex flex-col divide-y divide-slate-200 border-y border-slate-200 sm:flex-row sm:divide-x sm:divide-y-0">
                  {[
                    {
                      label: "Lớp đang học",
                      value: isLoadingClasses ? "—" : totalClasses,
                      hint: "Lớp bạn đã tham gia",
                      icon: GraduationCap,
                    },
                    {
                      label: "Nội dung",
