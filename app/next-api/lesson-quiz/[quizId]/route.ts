...
export async function PUT(req: Request, { params }: Params) {
  try {
    ...
    const { title, description, durationMinutes, passScore } = body;
    ...
    const res = await fetch(
      `${env.API_URL}${LESSON_QUIZ_API.UPDATE(quizId)}`,
      {
        method: "PUT",
        headers: {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { message: data?.message || "Update lesson quiz failed" },
        { status: res.status },
      );
    }
    ...
    console.error("UPDATE LESSON QUIZ ERROR:", error);
    return NextResponse.json(
      { message: "Internal server error" },
      { status: 500 },
    );
